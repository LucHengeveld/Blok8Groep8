...
def get_input():
    try:
        if request.method == 'POST':
            ...
            genepanel_file_path = os.path.abspath(genepanel_file)
            print(genepanel_file_path)
            ...
            # genepanel_file = "/home/christel/Downloads/GenPanelOverzicht_DG-3.1.0_HAN.xlsx"
            # genepanel_file = os.path.abspath(genepanel_file)
...
def make_gene_panel_dict(gps_set, genes_dict):
    ...
    for gene_panel in gps_set:
        gene_panel_dict[gene_panel] = [k for k, v in genes_dict.items()
                                       if
                                       gene_panel in v]
...
def read_pubtator_file(pubtator_link, gene_panel_dict,
                       genepanel_filter):
    ...
    for i in range(len(lines)):
        ...
        # mutationlist to the results dictionary.
            ...
            if genelist:
                results[article_id] = [title, abstract, genelist,
                                       diseaselist,
                                       mutationlist]
                genelist = []
                diseaselist = []
...
def co_occurrence(results, articlepoints, abstractpoints,
                  sentencepoints,
                  titlepoints, pos):
